...
{
    public sealed partial class SQLiteEstateService : IEstateBanServiceInterface, IEstateBanServiceListAccessInterface
    {
        List<UUI> IEstateBanServiceListAccessInterface.this[uint estateID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SQLiteCommand("SELECT UserID FROM estate_bans WHERE EstateID = @estateid", conn))
                    {
        ...
        bool IEstateBanServiceInterface.this[uint estateID, UUI agent]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SQLiteCommand("SELECT UserID FROM estate_bans WHERE EstateID = @estateid AND UserID LIKE '" + agent.ID.ToString() + "%'", conn))
                    {
                        cmd.Parameters.AddParameter("@estateid", estateID);
                        using (SQLiteDataReader reader = cmd.ExecuteReader())
                        {
