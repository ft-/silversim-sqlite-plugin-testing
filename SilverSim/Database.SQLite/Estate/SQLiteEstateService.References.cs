...
{
    public sealed partial class SQLiteEstateService : IAssetReferenceInfoServiceInterface
    {
        public void EnumerateUsedAssets(Action<UUID> action)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT DISTINCT CovenantID FROM estates", conn))
                {
