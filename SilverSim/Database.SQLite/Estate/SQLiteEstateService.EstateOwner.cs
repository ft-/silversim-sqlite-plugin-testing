...
{
    public sealed partial class SQLiteEstateService : IEstateOwnerServiceInterface
    {
        ...
        UUI IEstateOwnerServiceInterface.this[uint estateID]
        {
            ...
            set
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SQLiteCommand("UPDATE estates SET Owner = @ownerid WHERE ID = @id", conn))
                    {
                        cmd.Parameters.AddParameter("@id", estateID);
