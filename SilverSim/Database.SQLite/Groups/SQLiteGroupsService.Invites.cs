...
namespace SilverSim.Database.SQLite.Groups
{
    public sealed partial class SQLiteGroupsService : GroupsServiceInterface.IGroupInvitesInterface
    {
        ...
        bool IGroupInvitesInterface.ContainsKey(UUI requestingAgent, UUID groupInviteID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT InviteID from groupinvites WHERE InviteID = @inviteid", conn))
                {
        ...
        bool IGroupInvitesInterface.TryGetValue(UUI requestingAgent, UUID groupInviteID, out GroupInvite ginvite)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT * from groupinvites WHERE InviteID = @inviteid", conn))
                {
                    cmd.Parameters.AddParameter("@inviteid", groupInviteID);
                    using (SQLiteDataReader reader = cmd.ExecuteReader())
                    {
