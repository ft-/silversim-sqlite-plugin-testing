...
namespace SilverSim.Database.SQLite.Groups
{
    public sealed partial class SQLiteGroupsService : GroupsServiceInterface.IGroupRolesInterface
    {
        ...
        bool IGroupRolesInterface.ContainsKey(UUI requestingAgent, UGI group, UUID roleID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT r.GroupID FROM grouproles AS r WHERE r.GroupID = @groupid AND r.RoleID = @roleid", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
        ...
        bool IGroupRolesInterface.TryGetValue(UUI requestingAgent, UGI group, UUID roleID, out GroupRole groupRole)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT r.*, " + RCountQuery + " FROM grouproles AS r WHERE r.GroupID = @groupid AND r.RoleID = @roleid", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    cmd.Parameters.AddParameter("@roleid", roleID);
                    using (SQLiteDataReader reader = cmd.ExecuteReader())
