...
{
    public sealed partial class SQLiteInventoryService : IAssetReferenceInfoServiceInterface
    {
        public void EnumerateUsedAssets(Action<UUID> action)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT DISTINCT AssetID FROM " + m_InventoryItemTable + " WHERE NOT (AssetType = 24 OR AssetType = 25)", conn))
                {
