                ...
                using (var cmd = new SQLiteCommand("SELECT id, access_time FROM assetrefs WHERE id = @id", conn))
                {
                    ...
                    using (SQLiteDataReader dbReader = cmd.ExecuteReader())
                    {
                        if (dbReader.Read())
                        {
                            ...
                            {
                                /* update access_time */
                                using (var uconn = new SQLiteConnection(m_ConnectionString))
                                {
                                    uconn.Open();
                                    using (var ucmd = new SQLiteCommand("UPDATE assets SET access_time = @access WHERE id = @id", uconn))
                                    {
                ...
                using (var cmd = new SQLiteCommand(sql, dbcon))
                {
                    using (SQLiteDataReader dbReader = cmd.ExecuteReader())
                    {
                        while (dbReader.Read())
                        {
                            ...
                            {
                                /* update access_time */
                                using (SQLiteConnection uconn = new SQLiteConnection(m_ConnectionString))
                                {
                                    uconn.Open();
                                    using (SQLiteCommand ucmd = new SQLiteCommand("UPDATE assetrefs SET access_time = @access WHERE id = @id", uconn))
                                    {
                                        ucmd.Parameters.AddParameter("@access", Date.GetUnixTime());
                    ...
                    using (SQLiteDataReader dbReader = cmd.ExecuteReader())
                    {
                        if (dbReader.Read())
                        {
                            ...
                            if (asset.AccessTime - DateTime.UtcNow > TimeSpan.FromHours(1))
                            {
                                ...
                                {
                                    uconn.Open();
                                    using (var ucmd = new SQLiteCommand("UPDATE assetrefs SET access_time = @access WHERE id = @id", uconn))
                                    {
                                        ucmd.Parameters.AddParameter("@access", Date.GetUnixTime());
                                        ucmd.Parameters.AddParameter("@id", key);
                                        ucmd.ExecuteNonQuery();
