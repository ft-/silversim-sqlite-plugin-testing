...
namespace SilverSim.Database.SQLite.Profile
{
    public sealed partial class SQLiteProfileService : ProfileServiceInterface.INotesInterface
    {
        bool INotesInterface.ContainsKey(UUI user, UUI target)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT useruuid FROM usernotes WHERE useruuid = @user AND targetuuid = @target", conn))
                {
        ...
        bool INotesInterface.TryGetValue(UUI user, UUI target, out string notes)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT notes FROM usernotes WHERE useruuid = @user AND targetuuid = @target", conn))
                {
                    cmd.Parameters.AddParameter("@user", user.ID);
                    cmd.Parameters.AddParameter("@target", target.ID);
                    using (SQLiteDataReader reader = cmd.ExecuteReader())
