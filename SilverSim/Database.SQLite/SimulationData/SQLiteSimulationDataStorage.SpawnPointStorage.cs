...
{
    public sealed partial class SQLiteSimulationDataStorage : ISimulationDataSpawnPointStorageInterface
    {
        List<Vector3> ISimulationDataSpawnPointStorageInterface.this[UUID regionID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new SQLiteCommand("SELECT DistanceX, DistanceY, DistanceZ FROM spawnpoints WHERE RegionID = @regionid", conn))
                    {
