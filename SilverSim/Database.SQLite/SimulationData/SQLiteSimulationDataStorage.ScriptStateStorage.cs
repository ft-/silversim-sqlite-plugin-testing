...
{
    public sealed partial class SQLiteSimulationDataStorage : ISimulationDataScriptStateStorageInterface
    {
        bool ISimulationDataScriptStateStorageInterface.TryGetValue(UUID regionID, UUID primID, UUID itemID, out byte[] state)
        {
            using (var connection = new SQLiteConnection(m_ConnectionString))
            {
                ...
                using (var cmd = new SQLiteCommand("SELECT ScriptState FROM scriptstates WHERE RegionID = @regionid AND PrimID = @primid AND ItemID = @itemid LIMIT 1", connection))
                {
        ...
        byte[] ISimulationDataScriptStateStorageInterface.this[UUID regionID, UUID primID, UUID itemID]
        {
            ...
            set
            {
                using (var connection = new SQLiteConnection(m_ConnectionString))
                {
                    ...
                    connection.ReplaceInto("scriptstates", p);
                }
            }
        }
        ...
        bool ISimulationDataScriptStateStorageInterface.Remove(UUID regionID, UUID primID, UUID itemID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new SQLiteCommand("DELETE FROM scriptstates WHERE RegionID = @regionid AND PrimID = @primid AND ItemID = @itemid", connection))
                {
                    cmd.Parameters.AddParameter("@regionid", regionID);
                    cmd.Parameters.AddParameter("@primid", primID);
                    cmd.Parameters.AddParameter("@itemid", itemID);
