...
{
    public sealed partial class SQLiteSimulationDataStorage : ISimulationDataTerrainStorageInterface
    {
        List<LayerPatch> ISimulationDataTerrainStorageInterface.this[UUID regionID]
        {
            get
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new SQLiteCommand("SELECT PatchID, TerrainData FROM terrains WHERE RegionID = @regionid", connection))
                    {
