...
{
    public sealed partial class SQLiteGridService : IAssetReferenceInfoServiceInterface
    {
        public void EnumerateUsedAssets(Action<UUID> action)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SQLiteCommand("SELECT regionMapTexture, parcelMapTexture FROM " + m_TableName, conn))
                {
